...
| B13 | 产品服务 SQLite 连接使用上下文管理器但未显式关闭 | Windows 测试清理失败，长期运行可能积累数据库文件句柄 | ✅ 已修（连接上下文退出时显式 close） |
| B11 | 智云 CAS `loginView.sendsms.error` 触发 SMS 二次验证（2026-09） | 无法用账号密码登录智云 / 学在浙大，需要短信码；服务端策略问题 | 外部条件未解决；需浏览器完成验证后复用会话 |
| B12 | 学在浙大（Courses.zju.edu.cn）从这台机器公网 IP 直接访问失败，必须走 webvpn 或校园网；webvpn 入口又强制 SMS 二次验证 | 今天无法做学在浙大真实端到端 | 部分修复：会话 JSON 已修正，并持久化 WebVPN 模式；公网网络限制与短信验证仍需用户环境满足 |
...
| F4 | 设置页保存 LLM 后 `api_key_set` 未刷新 | 显示状态不准确 | ✅ 已修（后端返回并回显状态） |
...
| P3 | 质量报告无「已确认」状态回读 | 确认写入 `review/confirm-*.json`，前端未回显已确认状态 | ✅ 已修（报告接口返回并回显） |
| P4 | 学在浙大课件已下载到资料集，但生成 prompt 还没引用 | 文档里需与生成核心协调 | ✅ 已接入快照加载与章节上下文 |
...
- [x] 后端 78 个 unittest 全部通过，包含 SQLite 文件释放、上传标题持久化和 WebVPN 会话恢复回归。
- [x] 离线确定性验收全部通过。
- [x] 前端构建通过；页面级 Hook 依赖警告已减少，剩余为 UI 组件热更新提示和旧兼容页提示。
