...
各章：{json.dumps(compact, ensure_ascii=False)}"""
    ...
    # Apply patches
    def _chapter_key(ch) -> str:
        return ch.chapter_id or ch.lecture_id or ch.title

    patched: dict[str, LectureDraft] = {}
    for ch in chapters:
        key = _chapter_key(ch)
        patched.setdefault(key, ch.model_copy(deep=True))
    for item in data.get("chapter_patches") or []:
        if not isinstance(item, dict):
            continue
        key = str(item.get("chapter_id") or item.get("lecture_id") or "")
    ...
    ordered = [patched[_chapter_key(ch)] for ch in chapters]
    fallback = synthesize_book_fallback(course, ordered, plan=plan)
