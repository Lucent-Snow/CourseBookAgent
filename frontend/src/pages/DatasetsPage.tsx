...
import { ArrowRight, FileText, FolderOpen, Plus, Search, Settings } from 'lucide-react'
...
export function DatasetsPage() {
  ...
  const [setup, setSetup] = useState<{ llm: boolean; zhiyun: boolean; xuezai: boolean } | null>(null)
  ...
  useEffect(() => {
    void Promise.all([api.health(), productApi.providerAuth()]).then(([health, providers]) => {
      setSetup({
        ...
        xuezai: providers.xue_zai_zju.authenticated,
      })
    }).catch(() => undefined)
  }, [])
  ...
  return <div className="mx-auto max-w-[1180px] px-4 py-6 md:px-10 md:py-10">
    ...
    {setup && (!setup.llm || (!setup.zhiyun && !setup.xuezai)) && <div className="mt-5 flex flex-col items-start justify-between gap-3 rounded-md border border-amber-200 bg-amber-50 p-4 sm:flex-row sm:items-center"><div><p className="text-sm font-medium text-amber-900">完成基础配置后再开始生成</p><p className="mt-1 text-xs leading-5 text-amber-800">{!setup.llm ? '尚未配置大模型 API；' : ''}{!setup.zhiyun && !setup.xuezai ? '尚未连接智云或学在浙大账号。' : '当前仍可先上传本地资料。'}</p></div><Button render={<Link to="/settings" />} variant="outline" size="sm" className="shrink-0 border-amber-300 bg-white text-amber-900"><Settings size={14} />去系统设置</Button></div>}
