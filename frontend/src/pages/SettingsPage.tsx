...
export function SettingsPage() {
  ...
  useEffect(() => {
    void api.settings().then((value) => { setSettings(value); setBaseUrl(value.llm.base_url); setModel(value.llm.model); setInputPrice(value.llm.input_price_per_million?.toString() || ''); setOutputPrice(value.llm.output_price_per_million?.toString() || '') }).catch((err) => setError((err as Error).message))
    refresh()
  }, [])
  ...
  async function saveLlm() { await execute(async () => { const result = await api.saveLlm(baseUrl, model, apiKey, inputPrice.trim() ? Number(inputPrice) : undefined, outputPrice.trim() ? Number(outputPrice) : undefined); setBaseUrl(result.base_url); setApiKey(''); setSettings((current) => current ? { ...current, llm: { ...current.llm, base_url: result.base_url, model, configured: result.configured, api_key_set: result.api_key_set, input_price_per_million: result.input_price_per_million, output_price_per_million: result.output_price_per_million } } : current); setInputPrice(result.input_price_per_million?.toString() || ''); setOutputPrice(result.output_price_per_million?.toString() || ''); setMessage('模型配置已保存'); refresh() }) }
  ...
  return <div className="mx-auto max-w-[1080px] px-4 py-6 md:px-10 md:py-10"><header><p className="text-xs font-semibold text-[#147d86]">SYSTEM SETTINGS</p><h1 className="mt-2 text-[28px] font-semibold">系统设置</h1><p className="mt-2 text-sm text-[#718183]">配置数据源、模型和本地存储。运行创建后不会被这里的修改追溯覆盖。</p></header>{message && <div className="mt-5 flex items-center gap-2 rounded-md border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800"><CheckCircle2 size={16} />{message}</div>}{error && <div className="mt-5 rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</div>}<div className="mt-7 grid gap-5">
    ...
    <Section icon={<Bot size={17} />} title="模型与 Agent" description="配置 OpenAI 兼容模型；工作流在每次运行中选择并发与审校策略"><div className="grid grid-cols-2 gap-4"><label className="text-xs text-[#657678]">API 端点<Input className="mt-2" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} placeholder="https://api.example.com/v1" /></label><label className="text-xs text-[#657678]">模型名<Input className="mt-2" value={model} onChange={(e) => setModel(e.target.value)} placeholder="qwen-plus" /></label></div><label className="mt-4 block text-xs text-[#657678]">API Key<Input className="mt-2" type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder={settings?.llm.api_key_set ? '已设置；留空表示不修改' : 'sk-...'} /></label><div className="mt-4 grid grid-cols-2 gap-4"><label className="text-xs text-[#657678]">输入单价（元/百万 Token）<Input className="mt-2" type="number" min="0" step="any" value={inputPrice} onChange={(e) => setInputPrice(e.target.value)} placeholder="可选" /></label><label className="text-xs text-[#657678]">输出单价（元/百万 Token）<Input className="mt-2" type="number" min="0" step="any" value={outputPrice} onChange={(e) => setOutputPrice(e.target.value)} placeholder="可选" /></label></div><p className="mt-2 text-[11px] text-[#879496]">按你实际使用的模型或中转站价格填写；留空则只统计 Token，不估算成本。</p><div className="mt-4 flex gap-2"><Button disabled={busy} onClick={() => void saveLlm()}><Save size={14} />保存模型配置</Button><Button variant="outline" disabled={busy || !settings?.llm.configured} onClick={() => void execute(async () => { const result = await api.testLlm(); const usage = result.usage?.total_tokens ? ` · ${result.usage.total_tokens} Token` : ''; setMessage(`连接成功：${result.model} · ${result.latency_ms} ms${usage}`) })}>测试连接</Button></div></Section>
